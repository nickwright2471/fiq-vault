...
//
var imageSource = $.trim($("#inputUrl").val());
    if (imageSource != "") {
        alert(imageSource);
    }
...
//Warn users before leaving page
window.onbeforeunload = function(e) {
	e.returnValue = "WARNING. Leaving this page will reset the game and all progress will be lost!";
    return "WARNING. Leaving this page will reset the game and all progress will be lost!";
};


//if on mobile device, hide grid options so it defaults to 4x4
	if ($(window).width() < 768){
		$('.container-grid-option').css('display','none');
	}
	else{
		$('.container-grid-option').css('display','block');
	}
...
$(document).ready(function(){
	...
	var lastSavedGame = localStorage.getItem("savedGameDate");
	if(lastSavedGame != null){
		document.getElementById("lastSavedGame").innerHTML = "Last saved game: " + lastSavedGame + " <a id='enterLink' onclick='game.continue()'>ENTER</a> " ;
	}	
	else{
...
game.init = function()
{
	...
	//Since they are not loading the previous game, clear all values in the local storage.
	// clearStorage();
}
...
game.continue = function()
{
	...
	if(lastSavedGame != null){
		...
		//User clicked load but there isn't anything to load.
		window.alert("There are currently no saved games.");
	}	
}

game.loadSettings = function() {
	//option to remove header
	if ($("#removeHeader").is(":checked")) {
               $(".header").hide();
               $("#back-to-board").css("top","10px")
            }
    else {
    	$(".header").show();
    }
	...
	//Audio autoplays, and pauses when speaker is clicked
	// $('#audio').get(0).play();
 ...
 //        $('#audio').get(0).play();
 //        $('#play').attr('src','img/new-speaker-icon.png');
 //    } else {
 //        $('#audio').get(0).pause();
 //        $('#play').attr('src','img/new-speaker-mute-icon.png');
 //    }
	// });

	//Alex Trebek slides in and out
	$('.img-alex').css('animation','creep 5.4s ease 1.2s');
	setTimeout(function() { 
        $('.container-alex').hide();
 	}, 6600); 
	

	//Other elements fading in and out
	// $('#play').css('display','block');
	$('#save').css('display','block');
	$('body').css('background','rgb(42, 95, 152)');
	$('#game').fadeIn(1000);
	$('#options').hide();
	$('#copyright').hide();
	$('#stats').fadeIn(1000);
...
game.createScoreboard = function()
{
	...
	for(var i = 1; i <= game.team_cnt; i++)
	{
		...
		content += "<td><h3 id='team" + i +"'>0</h3><input class='subtract-points' onclick='game.subtractPoints(" + i +  ")' type='button' value='-' /><input class='add-points' onclick='game.addPoints(" + i +  ")' value='+' type='button' /> </td>";
	}
	content += "</tr></tbody></table>";
	// document.getElementById('team1').innerHTML = getTeamScore;
	$('#stats').html(content);
...
{	
	...
	//Since the question has been viewed and answered store question in the questions array.
	storeDirtyQuestion('#t' + game.current_questionID);
	storePointsInStorage();
...
game.save = function(){
	...
	//Confrim with the user if they would like to save
	var choice = window.confirm("Are you sure you want to save this session?\nYour saved game can be found on the home screen when you re-open the game.")
	if(choice == true){
	storeSettingsInStorage();
	storePointsInStorage();
	storeQuestionsInStorage();
	var date = new Date();
	localStorage.setItem("savedGameDate", (date.getMonth() + 1) + "/" + date.getDate() + "/" + date.getFullYear() + " " + date.getHours() + ":" + date.getMinutes());
	}	
}
...
prompt.show = function(questionID, points)
{
	...
	$('#game').hide()
	$('.height-container').css({"height": "100%"});
	$('body').css({"height": "calc(100vh + 74px)"});
...
prompt.hide = function()
{
	$('#prompt').hide();
	$('#game').show();
	$('.height-container').css({"height": "calc(100vh - 74px"});
...
prompt.showQuestion = function()
{
	...
	//Since the question has been viewed, store it in the question array.
	storeDirtyQuestion('#t' + game.current_questionID);
}

var questions = [];

function storeDirtyQuestion(question){
	var questionObj = {"questionID" : question};

	//Making sure after the user loads the game from previous save, that the game isn't storing null questions.
	//Example: User loads and immediately tries to add or subtract points but has not selected a question.
	if(question != "#tundefined"){
		questions.push(questionObj);
		storeQuestionsInStorage();
	}
}

function storeQuestionsInStorage(){
	//Store the questions array to storage as a string of objects.
	localStorage.setItem("dirtyQuestions", JSON.stringify(questions));
}

function loadDirtyQuestions(){
	//Convert the string of objects into an array of objects.
	questions = JSON.parse(localStorage.getItem("dirtyQuestions"));
	for(var i = 0; i < questions.length; i++){
		if(questions[i] != null){
			//Load the questions and make them "dirty".
			var dirtyQuestion = questions[i].questionID;
			$((dirtyQuestion)).addClass("dirty");	
			$((dirtyQuestion)).unbind('mouseover')
			$((dirtyQuestion)).unbind('mouseout')
		}
	}
}

function storePointsInStorage(){
	//Store team points in local storage
	for(var i = 1; i <= game.team_cnt; i++)
	{
	var teamScore = document.getElementById('team' + i).innerHTML;
  	localStorage.setItem("cookieTeamScore" + i, teamScore);
...
function loadSettingsFromStorage(){
	var headerVal = localStorage.getItem("header");
	var gridVal = localStorage.getItem("grid");
	var teamCount = localStorage.getItem("teamCount");	
	
	if(headerVal == "checked"){
		$("#removeHeader").checked = true;
	}
	$("#grid").val = gridVal;
	game.team_cnt= teamCount;
}

function loadPoints(){
	for(var i = 1; i <= game.team_cnt; i++)
	{
		var teamScore = localStorage.getItem("cookieTeamScore" + i);
		document.getElementById('team' + i).innerHTML = teamScore;
	}	
}

function storeSettingsInStorage(){
	localStorage.setItem("header", $("#removeHeader").val());
	localStorage.setItem("grid", $("#grid").val());
	localStorage.setItem("teamCount", game.team_cnt);
}
